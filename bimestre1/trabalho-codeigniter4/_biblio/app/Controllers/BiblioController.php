...
class BiblioController extends BaseController
{
    ...
    {   
      ...
      <script>
        ...
        function searchData(){
          window.location = 'index?search='+search.value
        }
      </script>
      </div>
      </div>
    ...
    public function listEmprest()
    {
      ...
      $query = $myModel->query('select relations.id, livros.title, usuarios.username, relations.dateupload from relations join livros on livros.id=relations.id_liv join usuarios on usuarios.id = relations.id_user;');
      $result = $query->getResultArray();
    ...
    public function emprestimosPage(){
      ...
      $emprestArray = $this->listEmprest();
      var_dump($emprestArray);
      $bodycontent = "<div class='listHeader'>
    ...
    public function deleteEmprest()
    {
      ...
      $bookOldQuant = $bookModel->find($bookid)['id'];
